...
#  ---------------------------------------------------------------------
"""
Off-center ("shifted") explicit discrete rigid body impact test.

The particle block sits off-axis relative to the rigid body, so contact is
eccentric and induces a genuine torque on the rigid body's reference point.
This is a regression test for the exact mass/inertia computation in
:func:`generateDiscreteRigidBodyFromMeshFile` /
:func:`computePolyhedronMassProperties`: a wrong (e.g. dimensionally
incorrect) rotary inertia lets the body spin unphysically fast under this
torque, which would blow up the block's contact displacement.
...
def run_sim(full: bool = False):
    ...
    # Shifts the block off the rigid body's central axis (along its cylinder
    # axis). Keeps the whole block within the rigid body's z in [-10, 10]
    # reach: half-width is BLOCK_SIZE * N / 2, so the offset must leave room.
    Z_OFFSET = 3.0 if full else 6.0
    ...
    # rigid_body.exo is a cylinder-like shape extruded along its local z-axis
    # (its cross-sectional gap to a probe point at x=0 is independent of z);
    # shifting the block along z therefore keeps it in contact while offsetting
    # the contact point far from the reference point -- inducing a real torque.
...
def test_sim(assert_gold):
    fieldOutputController = run_sim()

    res = fieldOutputController.fieldOutputs["displacement"].getLastResult()
    gold = np.loadtxt("gold_explicit_shifted_impact_block.csv")
    assert_gold(res, gold, atol=1e-10)

    # The rigid body must not spin unphysically fast under the eccentric
    # contact -- this bounds the regression against the dimensionally wrong
    # `inertia = [mass, mass, mass]` fallback that used to let it tip by
    # tens of degrees within a few hundredths of a second.
    rp_res = np.asarray(fieldOutputController.fieldOutputs["rigid_displacement"].getLastResult())
    assert np.all(np.isfinite(rp_res))
    assert np.max(np.abs(rp_res)) < 5.0
...
if __name__ == "__main__":
    ...
    fieldOutputController = run_sim(full=args.full)
    res = fieldOutputController.fieldOutputs["displacement"].getLastResult()

    if args.create_gold:
        if args.full:
            raise SystemExit("Refusing to overwrite the gold file with a --full run; run without --full.")
        np.savetxt("gold_explicit_shifted_impact_block.csv", np.asarray(res).flatten())
