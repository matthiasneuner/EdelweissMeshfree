...
#  ---------------------------------------------------------------------
"""
Basic explicit discrete rigid body contact test.

A rigid body (loaded from ``rigid_body.exo`` via
:func:`generateDiscreteRigidBodyFromMeshFile`) falls under a prescribed
initial velocity onto a particle block, exercised through the non-frictional
:class:`DiscreteRigidBodyPenaltyContactExplicit` constraint. Verifies that
contact is detected and penalty forces are transmitted into the block.
...
def run_sim(full: bool = False):
    ...
    # rigid_body.exo spans y=[10, 20] in its reference configuration; shift it
    # so its bottom sits just above the block's top face (at y=0).
...
def test_sim(assert_gold):
    ...
    gold = np.loadtxt("gold_explicit_rigid_body_contact.csv")
...
if __name__ == "__main__":
    ...
    if args.create_gold:
        ...
        np.savetxt("gold_explicit_rigid_body_contact.csv", np.asarray(res).flatten())
